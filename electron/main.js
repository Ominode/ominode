...
function saveAndOpenErrorLog() {
  try {
    ...
    const logFileName = `ominode-crash-${timestamp}.log`;
...
function getBinaryPath() {
  ...
  if (isDev) {
    const binaryName = platform === 'win32' ? 'ominode.exe' : 'ominode';
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    env.SQLITE_PATH = path.join(dataDir, 'ominode.db');
